...
func TestSetUpgradeInfo(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiSetUpgradeInfo, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := SetUpgradeInfo(tt.args.ctx, tt.args.payload)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
				t.Errorf("SetUpgradeInfo() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("SetUpgradeInfo() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
