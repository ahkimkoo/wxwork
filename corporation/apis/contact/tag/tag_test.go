...
func TestAddTagUsers(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiAddTagUsers, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := AddTagUsers(tt.args.ctx, tt.args.payload)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
				t.Errorf("AddTagUsers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("AddTagUsers() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
func TestDelTagUsers(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiDelTagUsers, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := DelTagUsers(tt.args.ctx, tt.args.payload)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
				t.Errorf("DelTagUsers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("DelTagUsers() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
